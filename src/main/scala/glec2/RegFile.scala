...
case class RegFile(implicit val conf : GlecCoreParams) extends Component {
  ...
  io.rs1_data := Mux(io.rs1_addr =/= U"0", regFile(io.rs1_addr), B"0");
  io.rs2_data := Mux(io.rs2_addr =/= U"0", regFile(io.rs2_addr), B"0");
  io.dm_rdata := Mux(io.dm_addr =/= U"0", regFile(io.dm_addr), B"0");
}
